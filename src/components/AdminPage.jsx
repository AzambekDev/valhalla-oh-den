  ...
  const calculateKPIs = (orderList) => {
    ...
    cleanOrders.forEach(order => {
      ...
      Object.keys(order.items || {}).forEach(name => {
        if (name.startsWith("_")) return; // skip metadata keys like _flags
            ...
            <tbody>
              {filteredOrders.length === 0 ? (
                ...
                paginatedOrders.map(order => {
                  const itemsCount = Object.entries(order.items || {}).reduce((sum, [name, qty]) => {
                    if (name.startsWith("_")) return sum;
                    return sum + (qty || 0);
                  }, 0);
                  return (
                    <tr key={order.id}>
